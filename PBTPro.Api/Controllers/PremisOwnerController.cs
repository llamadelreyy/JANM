/*
Project: PBT Pro
Description: Premis Owner controller
Author: Ismail
Date: June 2025
Version: 1.0
...
namespace PBTPro.Api.Controllers
{
    ...
    [ApiController]
    public class PremisOwnerController : IBaseController
    {
        private readonly ILogger<PremisOwnerController> _logger;
        private readonly string _feature = "PREMIS_OWNER";

        public PremisOwnerController(IConfiguration configuration, PBTProDbContext dbContext, PBTProTenantDbContext tenantDBContext, ILogger<PremisOwnerController> logger) : base(dbContext)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> GetList()
        {
            try
            {
                var data = await _tenantDBContext.mst_owner_premis.Where(x => x.is_deleted != true).OrderBy(x => x.owner_id).AsNoTracking().ToListAsync();
        ...
        public async Task<IActionResult> GetDetail(int Id)
        {
            try
            {
                var menu = await _tenantDBContext.mst_owner_premis.FirstOrDefaultAsync(x => x.owner_id == Id);
        ...
        [HttpPost]
        //[Route("Create")]
        public async Task<IActionResult> Add([FromBody] mst_owner_premi InputModel)
        {
            try
            {
                ...
                mst_owner_premi owner = new mst_owner_premi
                {
                ...
                _tenantDBContext.mst_owner_premis.Add(owner);
        ...
        [HttpPut("{Id}")]
        //[Route("Update")]
        public async Task<IActionResult> Update(int Id, [FromBody] mst_owner_premi InputModel)
        {
            try
            {
                int runUserID = await getDefRunUserId();
                ...
                #region Validation
                var owner = await _tenantDBContext.mst_owner_premis.FirstOrDefaultAsync(x => x.owner_id == Id);
                if (owner == null)
                {
                    return Error("", SystemMesg(_feature, "INVALID_RECID", MessageTypeEnum.Error, string.Format("Rekod tidak sah")));
                }
                ...
                _tenantDBContext.mst_owner_premis.Update(owner);
                await _tenantDBContext.SaveChangesAsync();
        ...
        public async Task<IActionResult> Remove(int Id)
        {
            try
            {
                ...
                try
                {
                    _tenantDBContext.mst_owner_premis.Remove(owner);
                    await _tenantDBContext.SaveChangesAsync();
                }
                catch (Exception ex)
                {
