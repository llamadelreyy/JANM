...
namespace PBTPro.Api.Controllers
{
    ...
    [ApiController]
    public class ShpAhliMajlisController : IBaseController
    {
        private readonly ILogger<ShpAhliMajlisController> _logger;
        private readonly string _feature = "SHP_AHLI_MAJLIS";
        private readonly int _defCRS = 4326;

        public ShpAhliMajlisController(PBTProDbContext dbContext, PBTProTenantDbContext tntdbContext, ILogger<ShpAhliMajlisController> logger) : base(dbContext)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> GetList(int? crs = null)
        {
            try
            {
                IQueryable<shp_ahli_majlis> initQuery = _tenantDBContext.shp_ahli_majlis.Where(x => PostGISFunctions.ST_IsValid(x.geom));

                if (crs != null && crs == _defCRS)
                {
                    initQuery = initQuery
                        .Select(x => new shp_ahli_majlis { id = x.id, AHLIMAJLIS = x.AHLIMAJLIS, AHLIMAJL0 = x.AHLIMAJL0, NO_TELEFON = x.NO_TELEFON, PENGGAL = x.PENGGAL, DUN = x.DUN, EKAR = x.EKAR, geom = (NetTopologySuite.Geometries.MultiPolygon)PostGISFunctions.ST_Transform(x.geom, crs.Value) });
                }

                var mst_lots = await initQuery
                .Select(x => new AhliMajlisPolygonViewModel
                {
                    gid = x.id,
                    name = x.AHLIMAJLIS,
                    ahli_majlis = x.AHLIMAJL0,
                    no_telefon = x.NO_TELEFON,
                    penggal = x.PENGGAL,
                    dun = x.DUN,
                    ekar = x.EKAR,
