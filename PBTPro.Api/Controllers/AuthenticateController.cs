        ...
        public async Task<IActionResult> Login([FromBody] LoginModel model)
        {
            try
            {
                ...
                if (LoginResult.Succeeded)
                {
                    ...
                    return Ok(new LoginResult {Userid = user.Id, Username = user.UserName, Token = token, Roles = roles.ToList() }, SystemMesg(_feature, "LOGIN", MessageTypeEnum.Success, string.Format("Log masuk berjaya.")));
                }
