...
namespace PBTPro.Api.Controllers
{
    ...
    [ApiController]
    public class ShpParlimentController : IBaseController
    {
        private readonly ILogger<ShpParlimentController> _logger;
        private readonly string _feature = "SHP_PARLIMENT";
        private readonly int _defCRS = 4326;

        public ShpParlimentController(PBTProDbContext dbContext, PBTProTenantDbContext tntdbContext, ILogger<ShpParlimentController> logger) : base(dbContext)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> GetList(int? crs = null)
        {
            try
            {
                IQueryable<shp_parliment> initQuery = _tenantDBContext.shp_parliments.Where(x => PostGISFunctions.ST_IsValid(x.geom));

                if (crs != null && crs == _defCRS)
                {
                    initQuery = initQuery
                        .Select(x => new shp_parliment { id = x.id, Name = x.Name, geom = (NetTopologySuite.Geometries.MultiPolygon)PostGISFunctions.ST_Transform(x.geom, crs.Value) });
                }

                var mst_lots = await initQuery
                .Select(x => new ParlimentPolygonViewModel
                {
                    gid = x.id,
                    name = x.Name,
