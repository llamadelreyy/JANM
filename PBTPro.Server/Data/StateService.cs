...
namespace PBTPro.Data
{
    ...
    public class StateService : IDisposable
    {
        ...
        private string _baseReqURL = "/api/States";
        private string LoggerName = "";
        private int LoggerID = 0;
        private int RoleID = 0;

        public StateService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor, ILogger<StateService> logger, PBTProDbContext dbContext, ApiConnector apiConnector, PBTAuthStateProvider PBTAuthStateProvider)
        {
        ...
        [HttpGet]
        public async Task<List<mst_state>> ListAll()
        {
        ...
        [HttpGet]
        public async Task<List<mst_state>> Refresh()
        {
            var result = new List<mst_state>();
            try
            {
        ...
        [HttpPost]
        public async Task<ReturnViewModel> Add(mst_state inputModel)
        {
        ...
        public async Task<ReturnViewModel> Update(int id, mst_state inputModel)
        {
            var result = new ReturnViewModel();
            try
            {
                ...
                string requestUrl = $"{_baseReqURL}/Update/{inputModel.state_id}";
        ...
        public async Task<mst_state> ViewDetail(int id)
        {
        ...
        public async Task<List<mst_state>> ViewListByCountry(string CountryCode)
        {
            ...
            try
            {
                ...
                string requestUrl = $"{_baseReqURL}/GetListByCountry{requestquery}";
                var response = await _apiConnector.ProcessLocalApi(requestUrl);
                ...
                if (response.ReturnCode == 200)
                {
                    ...
                    await _cf.CreateAuditLog((int)AuditType.Information, GetType().Name + " - " + MethodBase.GetCurrentMethod().Name, "Papar senarai mengikut negara.", LoggerID, LoggerName, GetType().Name, RoleID);
                }
                else
                {
                    await _cf.CreateAuditLog((int)AuditType.Error, GetType().Name + " - " + MethodBase.GetCurrentMethod().Name, "Ralat - Status Kod : " + response.ReturnCode, LoggerID, LoggerName, GetType().Name, RoleID);
        ...
        public async Task<mst_state> ViewDetailByCode(string Code)
        {
            var result = new mst_state();
            try
            {
