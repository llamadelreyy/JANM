...
{
    public partial class ConfiscationService : IDisposable
    {
        ...
        private List<trn_cfsc_view> _Sita { get; set; }

        private List<trn_cfsc> _cfsc {get; set;}
        public IConfiguration _configuration { get; }
        private readonly ApiConnector _apiConnector;
        private readonly PBTAuthStateProvider _PBTAuthStateProvider;
        protected readonly AuditLogger _cf;

        private string _baseReqURL = "/api/Confiscations";
        private string LoggerName = "";
        private int LoggerID = 0;
        private int RoleID = 0;

        public ConfiscationService(IConfiguration configuration, PBTProDbContext dbContext, ApiConnector apiConnector, PBTAuthStateProvider PBTAuthStateProvider)
        {
        ...
        [AllowAnonymous]
        [HttpGet]
        public Task<List<trn_cfsc_view>> GetCompoundAsync(CancellationToken ct = default)
        {
            ...
            return Task.FromResult(_Sita);
        }


        [HttpGet]
        public async Task<List<trn_cfsc_view>> ListReport()
        {
            var result = new List<trn_cfsc_view>();
            ...
            try
            {
                if (response.ReturnCode == 200)
                {
                    ...
                    {
                        result = JsonConvert.DeserializeObject<List<trn_cfsc_view>>(dataString);
                    }
        ...
        public async Task<ReturnViewModel> Update(trn_cfsc_view inputModel)
        {
            var result = new ReturnViewModel();
            try
            {
                int id = inputModel.id_sita;
