...
{
    [AllowAnonymous]
    public class RefPatrolTypeService : IDisposable
    {
        ...
        private readonly ILogger<RefPatrolTypeService> _logger;
        private readonly ApiConnector _apiConnector;
        private readonly PBTAuthStateProvider _PBTAuthStateProvider;

        private string _baseReqURL = "/api/RefPatrolType";
        private string LoggerName = "";
        private int LoggerID = 0;
        private int RoleID = 0;
        private List<ref_patrol_type> _ref_patrol_type { get; set; }

        public RefPatrolTypeService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor, ILogger<RefPatrolTypeService> logger, PBTProDbContext dbContext, ApiConnector apiConnector, PBTAuthStateProvider PBTAuthStateProvider)
        {
        ...
        [HttpGet]
        public async Task<List<ref_patrol_type>> ListAll()
        {
            var result = new List<ref_patrol_type>();
            ...
            try
            {
                if (response.ReturnCode == 200)
                {
                    ...
                    {
                        result = JsonConvert.DeserializeObject<List<ref_patrol_type>>(dataString);
                    }
            ...
            catch (Exception ex)
            {
                ...
                result = new List<ref_patrol_type>();
            }
            return result;
        }

        [HttpGet]
        public async Task<List<ref_patrol_type>> Refresh()
        {
            var result = new List<ref_patrol_type>();
            try
            {
        ...
        public async Task<ReturnViewModel> Add(ref_patrol_type inputModel)
        {
        ...
        public async Task<ReturnViewModel> Update(int id, ref_patrol_type inputModel)
        {
            var result = new ReturnViewModel();
            try
            {
                ...
                string requestUrl = $"{_baseReqURL}/Update/{inputModel.type_id}";
        ...
        public Task<List<ref_patrol_type>> GetRaceAsync(CancellationToken ct = default)
        {
            ...
            return Task.FromResult(_ref_patrol_type);
        }

        public async Task<ref_patrol_type> ViewDetail(int id)
        {
            var result = new ref_patrol_type();
            try
            {
                ...
                if (response.ReturnCode == 200)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(dataString))
                    {
                        result = JsonConvert.DeserializeObject<ref_patrol_type>(dataString);
                    }
