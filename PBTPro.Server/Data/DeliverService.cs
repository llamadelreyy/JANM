...
{
    public class DeliverService : IDisposable
    {
        ...
        private string _baseReqURL = "/api/Deliver";
        private string LoggerName = "";

        public DeliverService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor, ILogger<DeliverService> logger, PBTProDbContext dbContext, ApiConnector apiConnector, PBTAuthStateProvider PBTAuthStateProvider)
        {
        ...
        [HttpGet]
        public async Task<List<ref_deliver>> ListAll()
        {
            var result = new List<ref_deliver>();
            ...
            try
            {
                if (response.ReturnCode == 200)
                {
                    ...
                    {
                        result = JsonConvert.DeserializeObject<List<ref_deliver>>(dataString);
                    }
            ...
            catch (Exception ex)
            {
                ...
                result = new List<ref_deliver>();
            }
            return result;
        }

        [HttpGet]
        public async Task<List<ref_deliver>> Refresh()
        {
            var result = new List<ref_deliver>();
            try
            {
        ...
        [HttpPost]
        public async Task<ReturnViewModel> Add(ref_deliver inputModel)
        {
        ...
        public async Task<ReturnViewModel> Update(int Id, ref_deliver inputModel)
        {
            var result = new ReturnViewModel();
            try
            {
        ...
        public async Task<ref_deliver> ViewDetail(int id)
        {
            var result = new ref_deliver();
            try
            {
                ...
                if (response.ReturnCode == 200)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(dataString))
                    {
                        result = JsonConvert.DeserializeObject<ref_deliver>(dataString);
                    }
