...
{
    public class ConfiscationItemTypeService : IDisposable
    {
        ...
        private readonly ILogger<ConfiscationItemTypeService> _logger;
        private readonly ApiConnector _apiConnector;
        private readonly PBTAuthStateProvider _PBTAuthStateProvider;
        private string _baseReqURL = "/api/ConfiscationItem";
        private string LoggerName = "";
        private int LoggerID = 0;
        private int RoleID = 0;

        public ConfiscationItemTypeService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor, ILogger<ConfiscationItemTypeService> logger, PBTProDbContext dbContext, ApiConnector apiConnector, PBTAuthStateProvider PBTAuthStateProvider)
        {
        ...
        [HttpGet]
        public async Task<List<ref_cfsc_inventory>> ListAll()
        {
            var result = new List<ref_cfsc_inventory>();
            ...
            try
            {
                if (response.ReturnCode == 200)
                {
                    ...
                    {
                        result = JsonConvert.DeserializeObject<List<ref_cfsc_inventory>>(dataString);
                    }
                    await _cf.CreateAuditLog((int)AuditType.Information, GetType().Name + " - " + MethodBase.GetCurrentMethod().Name, "Papar semula senarai jenis barang sitaan.", LoggerID, LoggerName, GetType().Name, RoleID);
                }
                else
                {
            ...
            catch (Exception ex)
            {
                ...
                result = new List<ref_cfsc_inventory>();
            }
            return result;
        }

        [HttpGet]
        public async Task<List<ref_cfsc_inventory>> Refresh()
        {
            var result = new List<ref_cfsc_inventory>();
            try
            {
        ...
        [HttpPost]
        public async Task<ReturnViewModel> Add(ref_cfsc_inventory inputModel)
        {
        ...
        public async Task<ReturnViewModel> Update(int id, ref_cfsc_inventory inputModel)
        {
            var result = new ReturnViewModel();
            try
            {
                ...
                string requestUrl = $"{_baseReqURL}/Update/{inputModel.inv_id}";
        ...
        public async Task<ref_cfsc_inventory> ViewDetail(int id)
        {
            var result = new ref_cfsc_inventory();
            try
            {
                ...
                if (response.ReturnCode == 200)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(dataString))
                    {
                        result = JsonConvert.DeserializeObject<ref_cfsc_inventory>(dataString);
                    }
