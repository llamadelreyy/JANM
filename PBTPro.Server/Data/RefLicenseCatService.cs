...
namespace PBTPro.Data
{
    ...
    public class RefLicenseCatService : IDisposable
    {
        ...
        private readonly ILogger<RefLicenseCatService> _logger;
        private readonly ApiConnector _apiConnector;
        private readonly PBTAuthStateProvider _PBTAuthStateProvider;

        private string _baseReqURL = "/api/RefLicenseCategory";
        private string LoggerName = "";
        private int LoggerID = 0;
        private int RoleID = 0;
        private List<ref_license_cat> _ref_patrol_type { get; set; }
        
        public RefLicenseCatService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor, ILogger<RefLicenseCatService> logger, PBTProDbContext dbContext, ApiConnector apiConnector, PBTAuthStateProvider PBTAuthStateProvider)
        {
        ...
        [HttpGet]
        public async Task<List<ref_license_cat>> ListAll()
        {
            var result = new List<ref_license_cat>();
            ...
            try
            {
                if (response.ReturnCode == 200)
                {
                    ...
                    {
                        result = JsonConvert.DeserializeObject<List<ref_license_cat>>(dataString);
                    }
            ...
            catch (Exception ex)
            {
                ...
                result = new List<ref_license_cat>();
            }
            return result;
        }
    }
