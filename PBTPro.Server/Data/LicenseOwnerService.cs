...
{
    public class LicenseOwnerService : IDisposable
    {
        ...
        private readonly ILogger<LicenseOwnerService> _logger;
        private readonly ApiConnector _apiConnector;
        private readonly PBTAuthStateProvider _PBTAuthStateProvider;
        private string _baseReqURL = "/api/LicenseOwner";
        private string LoggerName = "";
        private int LoggerID = 0;
        private int RoleID = 0;

        public LicenseOwnerService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor, ILogger<LicenseOwnerService> logger, PBTProDbContext dbContext, ApiConnector apiConnector, PBTAuthStateProvider PBTAuthStateProvider)
        {
        ...
        [HttpGet]
        public async Task<List<mst_owner_licensee>> ListAll()
        {
            var result = new List<mst_owner_licensee>();
            ...
            try
            {
                if (response.ReturnCode == 200)
                {
                    ...
                    {
                        result = JsonConvert.DeserializeObject<List<mst_owner_licensee>>(dataString);
                    }
            ...
            catch (Exception ex)
            {
                ...
                result = new List<mst_owner_licensee>();
            }
            return result;
        }

        [HttpGet]
        public async Task<List<mst_owner_licensee>> Refresh()
        {
            var result = new List<mst_owner_licensee>();
            try
            {
        ...
        [HttpPost]
        public async Task<ReturnViewModel> Add(mst_owner_licensee inputModel)
        {
        ...
        public async Task<ReturnViewModel> Update(int id, mst_owner_licensee inputModel)
        {
            var result = new ReturnViewModel();
            try
            {
                ...
                string requestUrl = $"{_baseReqURL}/Update/{inputModel.owner_id}";
        ...
        public async Task<mst_owner_licensee> ViewDetail(int id)
        {
            var result = new mst_owner_licensee();
            try
            {
                ...
                if (response.ReturnCode == 200)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(dataString))
                    {
                        result = JsonConvert.DeserializeObject<mst_owner_licensee>(dataString);
                    }
