...
namespace PBTPro.Data
{
    ...
    public class DistrictService : IDisposable
    {
        ...
        private string _baseReqURL = "/api/Districts";
        private string LoggerName = "";

        public DistrictService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor, ILogger<DistrictService> logger, PBTProDbContext dbContext, ApiConnector apiConnector, PBTAuthStateProvider PBTAuthStateProvider)
        {
        ...
        [HttpGet]
        public async Task<List<mst_district>> ListAll()
        {
            var result = new List<mst_district>();
            ...
            try
            {
                if (response.ReturnCode == 200)
                {
                    ...
                    {
                        result = JsonConvert.DeserializeObject<List<mst_district>>(dataString);
                    }
            ...
            catch (Exception ex)
            {
                ...
                result = new List<mst_district>();
            }
            return result;
        }

        [HttpGet]
        public async Task<List<mst_district>> Refresh()
        {
            var result = new List<mst_district>();
            try
            {
        ...
        [HttpPost]
        public async Task<ReturnViewModel> Add(mst_district inputModel)
        {
        ...
        public async Task<ReturnViewModel> Update(int id, mst_district inputModel)
        {
            var result = new ReturnViewModel();
            try
            {
                ...
                string requestUrl = $"{_baseReqURL}/Update/{inputModel.district_id}";
        ...
        public async Task<mst_district> ViewDetail(int id)
        {
            var result = new mst_district();
            try
            {
                ...
                if (response.ReturnCode == 200)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(dataString))
                    {
                        result = JsonConvert.DeserializeObject<mst_district>(dataString);
                    }
            ...
            {
                result = new mst_district();
                await _cf.CreateAuditLog((int)AuditType.Error, GetType().Name + " - " + MethodBase.GetCurrentMethod().Name, ex.Message, 1, LoggerName, "");
            }
            return result;
        }

        public async Task<List<mst_district>> ViewListByState(string CountryCode)
        {
            var result = new List<mst_district>();
            try
            {
                string requestquery = $"/{CountryCode}";
                string requestUrl = $"{_baseReqURL}/GetListByState{requestquery}";
                var response = await _apiConnector.ProcessLocalApi(requestUrl);
                ...
                if (response.ReturnCode == 200)
                {
                    ...
                    await _cf.CreateAuditLog((int)AuditType.Information, GetType().Name + " - " + MethodBase.GetCurrentMethod().Name, "Papar senarai mengikut negeri.", 1, LoggerName, "");
                }
                else
                {
                    await _cf.CreateAuditLog((int)AuditType.Error, GetType().Name + " - " + MethodBase.GetCurrentMethod().Name, "Ralat - Status Kod : " + response.ReturnCode, 1, LoggerName, "");
        ...
        public async Task<mst_district> ViewDetailByCode(string Code)
        {
            var result = new mst_district();
            try
            {
