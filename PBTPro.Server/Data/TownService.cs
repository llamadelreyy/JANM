...
{
    public class TownService : IDisposable
    {
        ...
        private string _baseReqURL = "/api/Towns";
        private string LoggerName = "";
        private int LoggerID = 0;
        private int RoleID = 0;

        public TownService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor, ILogger<TownService> logger, PBTProDbContext dbContext, ApiConnector apiConnector, PBTAuthStateProvider PBTAuthStateProvider)
        {
        ...
        [HttpGet]
        public async Task<List<mst_town>> ListAll()
        {
            ...
            try
            {
                if (response.ReturnCode == 200)
                {
                    ...
                    await _cf.CreateAuditLog((int)AuditType.Information, GetType().Name + " - " + MethodBase.GetCurrentMethod().Name, "Papar senarai data.", LoggerID, LoggerName, GetType().Name, RoleID);
                }
                else
                {
        ...
        [HttpGet]
        public async Task<List<mst_town>> Refresh()
        {
            var result = new List<mst_town>();
            try
            {
        ...
        [HttpPost]
        public async Task<ReturnViewModel> Add(mst_town inputModel)
        {
        ...
        public async Task<ReturnViewModel> Update(int id, mst_town inputModel)
        {
            var result = new ReturnViewModel();
            try
            {
                ...
                string requestUrl = $"{_baseReqURL}/Update/{inputModel.town_id}";
        ...
        public async Task<mst_town> ViewDetail(int id)
        {
        ...
        public async Task<List<mst_town>> ViewListByDistrict(string DistrictCode)
        {
            ...
            try
            {
                ...
                string requestUrl = $"{_baseReqURL}/GetListByDistrict{requestquery}";
                var response = await _apiConnector.ProcessLocalApi(requestUrl);
                ...
                if (response.ReturnCode == 200)
                {
                    ...
                    await _cf.CreateAuditLog((int)AuditType.Information, GetType().Name + " - " + MethodBase.GetCurrentMethod().Name, "Papar senarai mengikut daerah.", LoggerID, LoggerName, GetType().Name, RoleID);
                }
                else
                {
                    await _cf.CreateAuditLog((int)AuditType.Error, GetType().Name + " - " + MethodBase.GetCurrentMethod().Name, "Ralat - Status Kod : " + response.ReturnCode, LoggerID, LoggerName, GetType().Name, RoleID);
        ...
        public async Task<mst_town> ViewDetailByCode(string Code)
        {
            var result = new mst_town();
            try
            {
                ...
                if (response.ReturnCode == 200)
                {
                    ...
                    await _cf.CreateAuditLog((int)AuditType.Information, GetType().Name + " - " + MethodBase.GetCurrentMethod().Name, "Papar data mengikut kod.", LoggerID, LoggerName, GetType().Name, RoleID);
                }
                else
                {
                    await _cf.CreateAuditLog((int)AuditType.Error, GetType().Name + " - " + MethodBase.GetCurrentMethod().Name, "Ralat - Status Kod : " + response.ReturnCode, LoggerID, LoggerName, GetType().Name, RoleID);
