...
namespace PBTPro.Data
{
    ...
    public class RaceService : IDisposable
    {
        ...
        private string _baseReqURL = "/api/Race";
        private string LoggerName = "";
        private List<ref_race> _Race { get; set; }

        public RaceService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor, ILogger<RaceService> logger, PBTProDbContext dbContext, ApiConnector apiConnector, PBTAuthStateProvider PBTAuthStateProvider)
        {
        ...
        [HttpGet]
        public async Task<List<ref_race>> ListAll()
        {
            var result = new List<ref_race>();
            ...
            try
            {
                if (response.ReturnCode == 200)
                {
                    ...
                    {
                        result = JsonConvert.DeserializeObject<List<ref_race>>(dataString);
                    }
            ...
            catch (Exception ex)
            {
                ...
                result = new List<ref_race>();
            }
            return result;
        }

        [HttpGet]
        public async Task<List<ref_race>> Refresh()
        {
            var result = new List<ref_race>();
            try
            {
        ...
        public async Task<ReturnViewModel> Add(ref_race inputModel)
        {
        ...
        public async Task<ReturnViewModel> Update(int id, ref_race inputModel)
        {
            var result = new ReturnViewModel();
            try
            {
                ...
                string requestUrl = $"{_baseReqURL}/Update/{inputModel.race_id}";
        ...
        public Task<List<ref_race>> GetRaceAsync(CancellationToken ct = default)
        {
        ...
        public async Task<ref_race> ViewDetail(int id)
        {
            var result = new ref_race();
            try
            {
                ...
                if (response.ReturnCode == 200)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(dataString))
                    {
                        result = JsonConvert.DeserializeObject<ref_race>(dataString);
                    }
