...
namespace PBTPro.Data
{
    ...
    public class NoticeDurationService : IDisposable
    {
        ...
        private string _baseReqURL = "/api/NoticeDuration";
        private string LoggerName = "";

        public NoticeDurationService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor, ILogger<NoticeDurationService> logger, PBTProDbContext dbContext, ApiConnector apiConnector, PBTAuthStateProvider PBTAuthStateProvider)
        {
        ...
        [HttpGet]
        public async Task<List<ref_ntc_duration>> ListAll()
        {
            var result = new List<ref_ntc_duration>();
            ...
            try
            {
                if (response.ReturnCode == 200)
                {
                    ...
                    {
                        result = JsonConvert.DeserializeObject<List<ref_ntc_duration>>(dataString);
                    }
                    await _cf.CreateAuditLog((int)AuditType.Information, GetType().Name + " - " + MethodBase.GetCurrentMethod().Name, "Papar semula senarai tempoh serahan.", 1, LoggerName, "");
                }
                else
                {
            ...
            catch (Exception ex)
            {
                ...
                result = new List<ref_ntc_duration>();
            }
            return result;
        }

        [HttpGet]
        public async Task<List<ref_ntc_duration>> Refresh()
        {
            var result = new List<ref_ntc_duration>();
            try
            {
        ...
        [HttpPost]
        public async Task<ReturnViewModel> Add(ref_ntc_duration inputModel)
        {
        ...
        public async Task<ReturnViewModel> Update(int Id, ref_ntc_duration inputModel)
        {
            var result = new ReturnViewModel();
            try
            {
        ...
        public async Task<ref_ntc_duration> ViewDetail(int id)
        {
            var result = new ref_ntc_duration();
            try
            {
                ...
                if (response.ReturnCode == 200)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(dataString))
                    {
                        result = JsonConvert.DeserializeObject<ref_ntc_duration>(dataString);
                    }
