...
namespace PBTPro.Data
{
    ...
    public class RefNoticeDurationService : IDisposable
    {
        ...
        private readonly ILogger<RefTrnStatusService> _logger;
        private readonly ApiConnector _apiConnector;
        private readonly PBTAuthStateProvider _PBTAuthStateProvider;

        private string _baseReqURL = "/api/RefNoticeDuration";
        private string LoggerName = "";
        private int LoggerID = 0;
        private int RoleID = 0;

        private List<ref_trn_status> _ref_trn_status { get; set; }

        public RefNoticeDurationService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor, ILogger<RefNoticeDurationService> logger, PBTProDbContext dbContext, ApiConnector apiConnector, PBTAuthStateProvider PBTAuthStateProvider)
        {
        ...
        [HttpGet]
        public async Task<List<ref_notice_duration>> ListAll()
        {
            var result = new List<ref_notice_duration>();
            ...
            try
            {
                if (response.ReturnCode == 200)
                {
                    ...
                    {
                        result = JsonConvert.DeserializeObject<List<ref_notice_duration>>(dataString);
                    }
            ...
            catch (Exception ex)
            {
                ...
                result = new List<ref_notice_duration>();
            }
            return result;
        }
    }
